    ...
    @if (!$journals->isEmpty())
      @foreach ($journals as $journal)
        ...
        <div class="space-y-4 border-b border-slate-300 pb-4">
          <div class="space-y-2">
            <h1 class="font-bold text-[#14b321] uppercase">{{ $journal->title }}</h1>
          ...
          <div class="flex sm:items-center gap-4 max-sm:flex-col">
            ...
            'hashedId' => $hashedId,
              ...
              <button type="button"
                class="inline-flex justify-center items-center gap-2 whitespace-nowrap rounded-md border border-[#14b321] px-3 py-1.5 text-xs font-medium tracking-wide transition hover:opacity-75 text-center focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#14b321] active:opacity-100 active:outline-offset-0 disabled:opacity-75 disabled:cursor-not-allowed">
                <img src="{{ asset('images/pdf.png') }}" class="w-5 object-contain">
                PDF Open Access
              </button>
            </a>
            <p class="font-semibold">Page No.: <span class="text-[#14b321]">{{ $journal->page_number }}</span></p>
          </div>
        </div>
      @endforeach
    @else
