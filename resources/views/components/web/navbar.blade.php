...
<div class="hidden md:block">
  <div class="max-w-6xl mx-auto py-4 border-b border-slate-300 flex items-center justify-between gap-4">
    ...
    @if (Auth::check())
      <a href="{{ route('dashboard') }}" class="font-semibold hover:text-[#14b321]">Dashboard</a>
    @else
      <a href="{{ route('login') }}" class="font-semibold hover:text-[#14b321]">Login</a>
    @endif
  </div>
</div>
        ...
        @foreach ($items as $item)
          @if (isset($item['subitems']))
            <div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
              <button class="flex items-center gap-2 hover:text-[#14b321] font-semibold cursor-pointer"
                @click="open = !open" :aria-expanded="open">
                {{ $item['name'] }}
                <i data-lucide="chevron-down" class="size-5 shrink-0 transition" stroke-width="1.5"></i>
              </button>
              <div
                class="absolute min-w-[280px] left-1/2 top-full -translate-x-1/2 mt-4 bg-[#14b321] shadow-md z-50 rounded-md pt-0.5"
                ...
                <li class="flex flex-col gap-4 bg-white px-6 py-4 rounded-md">
                  @foreach ($item['subitems'] as $subitem)
                    <a href="{{ $subitem['route'] }}" class="font-semibold hover:text-[#14b321]">
                      {{ $subitem['name'] }}
                    </a>
                  @endforeach
                </li>
          ...
          @else
            <li>
              <a href="{{ route($item['route']) }}" class="font-semibold hover:text-[#14b321]">
                {{ $item['name'] }}
              </a>
            </li>
          @endif
